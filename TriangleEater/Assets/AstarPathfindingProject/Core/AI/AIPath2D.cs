...
public class AIPath2D : MonoBehaviour
{
    ...
    public virtual void OnTargetReached ()
    {
        ...
        {
            // disable the old path
            this.OnDisable();

            // find a new waypoint to travel to
            target = chooseRandomWaypoint(patrolPoints);

            // start the path to the new target
            this.Start();
        }
    ...
    public virtual void Update ()
    {
        ...
        {
            // walk only if we are within the sight range
            if (Vector3.Distance(tr.position, target.position) < sightRange)
            {
                walk();
            }

            // otherwise the enemy should switch to alert
            else
            {
                alert();
            }
    ...
    /**
    * Returns a random waypoint from an array of transform waypoints
    **/
    Transform chooseRandomWaypoint(Transform[] wayPoints)
    {
        int chosenVal = Random.Range(0, wayPoints.Length);

        // update current waypoint and return target location
        // currentWaypointIndex = chosenVal;
        return wayPoints[chosenVal];
    }

    /** 
    * Returns the patrol point nearest to the seeker
    **/
    Transform getClosestPatrolPoint(Transform[] wayPoints)
    {
        ...
        for (int i = 0; i < wayPoints.Length; i++)
        {
            ...
            float curDistance = Vector3.Distance(wayPoints[i].position, transform.position);

            // if this is the first time, the current distance IS the closest distance
            if (i == 0) closestDistance = curDistance;
    ...
    void pivot(Vector3 target)
    {
        ...
        transform.rotation = Quaternion.Slerp(sight.rotation, _lookRotation, Time.deltaTime * rotationSpeed);
    }

    public void chase(Transform newTarget)
    {
        // change status to chase
        ...
        this.Start();
        // when the target is out of range, become alert
        // alert();
    }
    ...
    void OnDrawGizmos()
    {
        if (patrolPoints != null)
        {
            ...
            {
                Vector3 globalWaypointPos = (Application.isPlaying) ? patrolPoints[i].position : patrolPoints[i].position + transform.position;
